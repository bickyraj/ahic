...
class AgentInformationController extends Controller
{
    ...
    {
        $agents = AgentInformation::whereIn('agent_request', [0, 1])->with('documents')->get();
    ...
    {
        $agents = AgentInformation::where('agent_request', '=', 2)->with('documents')->get();
        foreach ($agents as $agent) {
            //for each agents as agent
            if ($agent->documents != null) {
                //checking if agent has company
...
//
     ...
     */
     ...
     {
        $status = 0;
         $e = AgentInformation::findOrFail($id);
         $e->agent_request = $request->status;
