    ...
    <b-row>
      <b-col class="col-md-8">
        <b-card class="mb-2 trump-card">
          <div class="card-title">
            <div class="caption">
              <h5><i class="fas fa-key"></i> Why Study Ahic </h5>
            </div>
            <div class="caption card-title-actions">
              <b-button @click="showModal" variant="primary" class="btn btn-sm green pull-right">Add New</b-button>
              <b-modal class="ess-modal" ref="myModalRef" hide-footer title="Add New - Why study in Ahic">
                <form @submit.prevent="addNews" ref="addNewsForm" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                    ...
                    <select name="link" class="form-control">
                      ...
                      <optgroup v-for="item in allCagtegoriesCourses" :value="item.link" :key="item.id" :label="item.name">
                        <option v-for="course in item.courses" :value="course.courseLink" :key="course.id"> {{course.name}}</option>
                      </optgroup>
                    </select>
                  </div>
                  <div class="form-group">
                  ...
                  <b-btn class="mt-3 pull-right" variant="primary" type="submit">Create</b-btn>
            ...
            <div class="col-md ml-auto mr-auto">
              <nav aria-label="Page navigation" class="ml-auto mr-auto">
                <ul class="pagination">
                  ...
                  <li class="page-item" v-if="pagination.current_page != pagination.last_page"><a href="javascript:;">...{{ pagination.last_page }}</a></li>
                  <li class="page-item">
      ...
      <b-col md="4" v-if="view != null">
        <b-card class="mb-2 trump-card">
          ...
          <img v-if="view.icon_name" :src="'../public/images/ahic-studies/'+view.icon_name" class="col-md-12" alt="">
          <div class="col-md-12">
            <h4>{{view.title}}</h4>
      ...
      <form @submit.prevent="editNews" :row="modalInfo.row" ref="editNewsForm">
        ...
        <div class="form-group">
          ...
          <select name="link" class="form-control">
            <option value="" selected> --Choose Link-- </option>
            <optgroup v-for="item in allCagtegoriesCourses" :value="item.link" :key="item.id" :label="item.name">
              <option v-for="course in item.courses" :value="course.courseLink" :selected="modalInfo.data.link == course.courseLink" :key="course.id"> {{course.name}}</option>
            </optgroup>
          </select>
        </div>
        <!-- <div class="form-group" v-if="modalInfo.data.icon_name == null">
          <label for="">Image </label>
...
<script>
  export default {
    ...
    computed: {
      ...
      cropimage() {
        if (this.modalInfo.data.icon_name != null) {
          this.myCroppa.refresh()
          return '../public/images/ahic-studies/' + this.modalInfo.data.icon_name
        }
      },
      filteredData() {
    ...
    methods: {
      ...
      updateStatus(id, status) {
        var self = this;
        let url = self.$root.baseUrl + '/api/admin/study-ahic/update';
        var formData = {
          id: id,
          status: status
        };
      ...
      fetchNews() {
        ...
        axios.get(vm.url || vm.$root.baseUrl + '/api/admin/study-ahic')
      ...
      fetchCourseCategories() {
        let vm = this;
        // let url = vm.$root.baseUrl + '/api/admin/testimonials';
        axios.get(vm.url || vm.$root.baseUrl + '/api/admin/get-all-categories-with-courses')
          .then(function(response) {
            vm.allCagtegoriesCourses = response.data.data;
          })
          .catch(function(error) {
            console.log(error);
          });
      ...
      addNews() {
        ...
        let url = self.$root.baseUrl + '/api/admin/study-ahic';
        formData.append('image', this.myCroppa.generateDataUrl())
        axios.post(url, formData).then(function(response) {
            self.fetchNews();
            $(form)[0].reset();
            self.hideModal();
            self.$toastr.s("A new study has been added.");
          })
          .catch(function(error) {
            self.error = '';
      ...
      editNews: function() {
        ...
        var formData = new FormData(form);
        let url = self.$root.baseUrl + '/api/admin/study-ahic/edit';
        formData.append('image', this.myCroppa.generateDataUrl())
        axios.post(url, formData).then(function(response) {
            self.fetchNews();
            self.hideNewsModal();
