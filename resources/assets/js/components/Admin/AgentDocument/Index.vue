      ...
      <b-col>
        <b-card class="mb-2 trump-card">
          <div class="card-title">
            <div class="caption">
              <h5><i class="fas fa-key"></i> Agent Document </h5>
            </div>
            <div class="caption card-title-actions">
              <b-button @click="showModal" variant="primary" class="btn btn-sm green pull-right">Add Agent Document</b-button>
              <b-modal class="ess-modal" ref="myModalRef" hide-footer title="Add Course">
                <form @submit.prevent="addAgentDoc" ref="addAgentDocForm">

                    <div class="form-group">
                    <label for=""> Agent</label>
                      <select name="agent_id" class="form-control">
                          <option value="">Select A Agent</option>
                          <option  v-for="agent in agents" :value="category.id" :key="agent.id">{{agent.first_name}}</option>
                      </select>
                  </div>

                  <div class="form-group">
                    <label for="">EOI </label>
                    <input type="file" name="EOI" class="form-control" placeholder="" required>
                  </div>
                
                  <div class="form-group">
                    <label for="">ABN/Registration Doc </label>
                    <input type="file" name="ABN" class="form-control" placeholder="" required>
                  </div>
                
                  <div class="form-group">
                    <label for="">Reference</label>
                    <input type="file" name="reference" class="form-control" placeholder="" required>
                  </div>
                
                  <div class="form-group">
                    <label for="">Qualification</label>
                    <input type="file" name="qualification" class="form-control" placeholder="" >
                  </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      addAgentDoc: function() {
        var self = this;
        var form = self.$refs.addAgentDocForm;
        var formData = new FormData(form);
        let url = self.$root.baseUrl + '/api/admin/course';
        axios.post(url, formData).then(function(response) {
