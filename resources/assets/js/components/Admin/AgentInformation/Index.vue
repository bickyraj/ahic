              ...
              <b-tabs content-class="" fill>
                ...
                <b-tab title="Requests">
                  <table class="table trump-table table-hover">
                    ...
                    <tbody v-if="table_request_items.length > 0" v-show="!loading">
                      <tr v-for="(agent, index) in table_request_items" :key="agent.id">
                        <td>{{ agent.first_name}} {{agent.last_name}} </td>
                        <td>{{ agent.address.substring(0,37) + ".."}}</td>
                        <td>
                          <select name="" class="form-control form-control-sm" @change="updateRequestStatus(agent.id, index, $event.target)">
                            <option value="2" :selected="agent.agent_request == 2">pending</option>
                            <option value="1" :selected="agent.agent_request == 1">accept</option>
                            <option value="3" :selected="agent.agent_request == 3">reject</option>
                          </select>
                        </td>
                      </tr>
                    </tbody>
...
<script>
  export default {
    ...
    methods: {
      updateRequestStatus(item, index, el) {
        let self = this;
        let url = self.$root.baseUrl + '/api/admin/agent/updateStatus/' + item;
        var status = el.value;
        axios.post(url, {
          status: status
        }).then(response => {
      ...
      fetchRequestAgents() {
        let vm = this;
        let self = this;
        let url = self.$root.baseUrl + '/api/admin/agent_request_informations';
        axios.get(url)
          .then(function(response) {
            vm.table_request_items = response.data.data;
            vm.loading = false;
          })
          .catch(function(error) {
            // console.log(error);
