    ...
    <div class="container">
        <div class="navbar p-0 navbar-expand-lg">
            ...
            <div class="collapse navbar-collapse when-collapsed" id="ec-nav__collapsible">
                <ul class="nav navbar-nav ec-nav__navbar ml-auto theme-nav">
                    <?php foreach ($menus as $menu): ?>
                    @if ($menu['parent_page']['slug'] =='courses')
                    <li class="nav-item nav-item__has-dropdown">
                        <a href="{{url('/')}}/{{$menu['parent_page']['slug']}}" class="nav-link text-capitalize dropdown-toggle <?= ((\Request::segment(1) == $menu['parent_page']['slug'])?'active': ''); ?>" role="button" aria-haspopup="true" aria-expanded="false">
                            {{$menu['name']}} 
                            <!-- {{ \Request::segment(1) }} -->
                    ...
                    @elseif ($menu['parent_page']['slug'] == 'admission' || $menu['parent_page']['slug'] =='student-service')
                    <li class="nav-item nav-item__has-dropdown">
                        <a href="{{url('/')}}/{{$menu['parent_page']['slug']}}" class="nav-link text-capitalize dropdown-toggle <?= ((\Request::segment(1) == $menu['parent_page']['slug'])?'active': ''); ?>" role="button" aria-haspopup="true" aria-expanded="false">
                            {{$menu['name']}}
                            <span class="caret"></span></a>
                        <ul class="dropdown-menu ">
                            @foreach ($module_nav as $req_module)
                                @if ($req_module['page_id'] == $menu['page_id'])
                                    <li class=""><a href="{{url('/')}}/{{$menu['parent_page']['slug']}}/{{$req_module['module']['title']}}" class="nav-link__list">
                                        {{$req_module['module']['title']}}
                                    </a></li>
                                @endif
                            @endforeach
                        </ul>
                    ...
                    <li class="nav-item nav-item__has-dropdown">
                        ...
                        <ul class="dropdown-menu ">
                            ...
                            {{-- <li class=""><a href="/{{$menu['parent_page']['slug']}}/{{$name}}" class="nav-link__list"> --}}
                            <li class=""><a href="{{url('/')}}/{{$menu['parent_page']['slug']}}/{{$submenus['parent_page']['slug']}}" class="nav-link__list">
                                    {{$submenus['name']}}
                                </a></li>
                            @endforeach
                        </ul>
